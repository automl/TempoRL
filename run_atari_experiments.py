...
class DQN:
    ...
    def __init__(self, state_dim: int, action_dim: int, gamma: float,
        ...
        # self._q_optimizer = optim.RMSprop(self._q.parameters(), lr=0.00025, alpha=0.01, eps=1e-08, momentum=0.95)
    ...
    def train(self, episodes: int, max_env_time_steps: int, epsilon: float, eval_eps: int = 1,
        ...
        for e in range(episodes):
            ...
            for t in range(max_env_time_steps):
                ...
                if (total_steps > learning_starts) and (total_steps % self.train_freq == 0):
                    num_update_steps += 1
                    ...
                    if (total_steps % target_net_upd_freq) == 0:
...
class DAR:
    ...
    def __init__(self, state_dim: int, action_dim: int,
                 ...
                 gamma: float, env: gym.Env, eval_env: gym.Env, vision=False):
        """
        Initialize the DQN Agent
        :param state_dim: dimensionality of the input states
        :param action_dim: dimensionality of the output actions
    ...
    def get_action(self, x: np.ndarray, epsilon: float) -> int:
        ...
        Simple helper to get action epsilon-greedy based on observation x
        """
        u = np.argmax(self._q(tt(x[None, :])).detach().numpy())
        r = np.random.uniform()
        if r < epsilon:
            return np.random.randint(self._action_dim)
        return u
...
class TDQN:
    ...
    def __init__(self, state_dim, action_dim, skip_dim, gamma, env, eval_env, vision=False, shared=True):
        ...
        if shared:
            ...
            if not vision:
                ...
                self._skip_q = NatureTQN(state_dim, skip_dim).to(device)
    ...
    def get_skip(self, x: np.ndarray, a: np.ndarray, epsilon: float) -> int:
        ...
        """
        u = np.argmax(self._skip_q(tt(x[None, :]), tt(a[None, :])).detach().numpy())
        r = np.random.uniform()
        if r < epsilon:
...
if __name__ == "__main__":
    ...
    # tqn -> Not usable with vision states
    ...
    parser.add_argument('--env',
                        type=str,
                        help="Possible envs = 'mountain', 'moon' or any Atari env",
                        default='mountain')
    ...
    if args.env not in ['mountain', 'moon']:
        ...
        if args.agent == 'dqn':
            ...
            agent = TDQN(state_dim, action_dim, args.skip_net_max_skips, gamma=0.99, env=env,
                         eval_env=eval_env, shared=False, vision=True)
        elif args.agent == 'dar':
